...
sleep 2 && mkdir /mnt/pi && sleep 2 && sudo mount ${blkidDev}3 /mnt/pi
sleep 2 && mkdir /mnt/boot && sleep 2 && sudo mount ${blkidDev}1 /mnt/boot
...
#Funtoo
	...
	#then
		txzExtract "${owd}/portage-latest-*.tar.xz" "/mnt/pi/usr"
	#else
#		wget -O ${owd}/portage-latest-${utc}.tar.xz http://ftp.osuosl.org/pub/funtoo/funtoo-current/snapshots/portage-latest.tar.xz
#		txzExtract "${owd}/portage-latest-${utc}.tar.xz" "/mnt/pi/usr"
	#fi
	cd /mnt/pi/usr/portage
